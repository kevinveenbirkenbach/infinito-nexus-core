...
| Category | Command | What it does | When to use it |
|---|---|---|---|
| Setup remotes | `make setup-remotes` | Configures the fork-based remote layout via [setup-remotes.sh](../../../scripts/git/setup-remotes.sh): `origin` points at the canonical repo, `fork` points at the personal fork, `main` tracks `origin/main`, and `remote.pushDefault` is set to `fork` so new branches land on the fork. Idempotent. Refuses to run inside the Claude sandbox because it writes `.git/config`. Accepts `FORK_URL=...` when the clone came from canonical and no fork URL is locally known yet. | Run once after cloning, and any time the remote layout drifts. See [remotes.md](../artefact/git/remotes.md) for the full rationale. |
| Sign and push | `make sign-push` | GPG-signs every unpushed commit on the current branch via [sign-push.sh](../../../scripts/git/sign-push.sh) and pushes (force-with-lease on rebased history). For a branch with no upstream, the push target is resolved from `remote.pushDefault` (falling back to `origin`), so the fork-based workflow from [remotes.md](../artefact/git/remotes.md) routes new branches to the personal fork instead of the canonical repo. Refuses to run inside the Claude sandbox and when the working tree is dirty. | Use this instead of `git push`. Direct push is denied in [settings.json](../../../.claude/settings.json) so that only the operator (with access to the GPG key in `~/.gnupg`) ever signs and ships commits. Agents MUST instruct the operator to run this target rather than pushing themselves. |
