...
def _read_node_onion(base_dir: Path) -> str:
    """The provisioned node onion address, read from the authoritative key file
    (``.onion-identity/hs/hostname`` — the same source ``services.tor.node`` is
    provisioned from); empty when no onion identity is present."""
    hostname = identity_hs_dir(base_dir) / "hostname"
    try:
        return hostname.read_text(encoding="ascii").strip()  # nocheck: cache-read
    except OSError:
        return ""
...
def iter_vhost_files_for_entity(
    ...
    *,
    ...
    to *entity*, plus the ``www.<domain>.conf`` redirect variant and — when a
    node onion is provisioned (``services.tor.node`` / the key file) — the
    dual-stack ``<sub>.<node-onion>.conf`` variant, so a dropped app leaves no
    stale onion vhost either. Only yields files that exist.
    """
    ...
    node_onion = _read_node_onion(roles_dir.parent)
